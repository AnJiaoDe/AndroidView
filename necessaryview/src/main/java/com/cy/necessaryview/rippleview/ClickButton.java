...
public class ClickButton extends Button {
    ...
    public ClickButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.ClickButton);
        //水波纹的颜色,默认是0x66000000，建议自定义水波纹颜色的时候，用argb,rgb都设置为0，a可随意，调整透明度为了水波纹看起来更美观
        int colorRipple = arr.getColor(R.styleable.ClickButton_colorRipple, 0x66000000);
        boolean havaRipple = arr.getBoolean(R.styleable.ClickButton_haveRipple, true);//设置是否有水波纹点击效果，默认有
        arr.recycle();
        //5.0以上才有效,
